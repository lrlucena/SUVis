...
/**
 * The SVColorMaps class represents various colormaps that can be used for data
 * visualization using color scale like the SU program suximage.
 *
...
public class SVColorMaps {
    ...
    public void updateData(float[][] pData, int idata, float pF1, float pD1, int pN1, float pF2, float pD2, int pN2) {
        ...
        arraycopy(pData[idata], 0, m_data, 0, ndata);
    ...
    public void updateData(float[] pData, float pF1, float pD1, int pN1, float pF2, float pD2, int pN2) {
        ...
        System.arraycopy(pData, 0, m_data, 0, pData.length);
    ...
    private void dataToByte(float[] pData) {
        try {
            ...
            if (balance == 0) {
                ...
                for (iz = 0; iz < nz; ++iz) {
                    m_temp[iz] = abs(pData[iz]/*m_data[iz]*/);
                }
            }
 ...
 /* define hue, saturation, lightness values */
    ...
    private final int m_bytesPerPixel = 3;
    private final int m_byteOrder = LSBFirst;
