...
abstract public class SVActor {
    ...
    private int getPositionValueAt(float x1, float x2) {
        ...
        int XMatr = round((x1 - XlengthHI) / distanciax) + 1;
        int YMatr = round((x2 - delrt) / dt) + 1;
...
//  cwp_Bool istmpdir=cwp_false;/* true for user given path               */
        ...
        /* Data validation */
        if (trap < 0.0) {
            showMessageDialog(null, format("trap = %f, must be positive.%nGain will not be applied.", trap));
            return false;
        }

        if (clip < 0.0) {
            showMessageDialog(null, format("clip = %f, must be positive.%nGain will not be applied.", clip));
            return false;
        }
        if (qclip < 0.0 || qclip > 1.0) {
            showMessageDialog(null, format("qclip = %f, must be between 0 and 1.%nGain will not be applied.", qclip));
            return false;
        }
        if (agc || gagc) {
            iwagc = (int) (wagc / m_d1);
            if (iwagc < 1) {
                showMessageDialog(null, format("wagc=%g must be positive.%nGain will not be applied.", wagc));
                return false;
            }
            if (iwagc > m_n1) {
                showMessageDialog(null, format("wagc=%g too long for trace.%nGain will not be applied.", wagc));
                return false;
            }
            iwagc >>= 1;
            /* windows are symmetric, so work with half */
        }
        if (jon) {
            tpow = (float) 2.0;
            gpow = (float) 0.5;
    ...
    public void gain(float[] data, float tpow, float epow, float gpow,
        ...
        if (!CLOSETO(gpow, f_one)) {
            ...
            if (CLOSETO(gpow, f_half)) {
                for (i = 0; i < nt; ++i) {
                    val = data[i];
                    data[i] = (float) ((val >= 0.0) ? sqrt(val) : -sqrt(-val));
                }
                ...
                for (i = 0; i < nt; ++i) {
                    val = data[i];
                    data[i] = (float) ((val >= 0.0) ? pow(val, gpow) : -pow(-val, gpow));
                }
            }
        }
        if (agc) {
            do_agc(data, iwagc, nt);
        }
        if (gagc) {
            do_gagc(data, iwagc, nt);
        }
        if (trap > 0.0) {
            do_trap(data, trap, nt);
        }
        if (clip > 0.0) {
...
//  static cwp_Bool first = cwp_true;   /* first entry flag     */
        ...
        if (first) {
            /* first entry, set up array of tpow factors */
            tpowfac = new float[nt];
            /* protect against negative tpow */
            tpowfac[0] = (float) ((tmin == 0.0) ? 1.0 : pow(tmin, tpow));
            for (i = 1; i < nt; ++i) {
                tpowfac[i] = (float) pow(tmin + i * dt, tpow);
            }
    ...
    void do_epow(float[] data, /* the data                     */
        ...
        if (first) {
            ...
            for (i = 0; i < nt; i++) {
                epowfac[i] = (float) exp(epow * (tmin + i * dt));
            }
...
//    dataptr++;
    ...
    void do_qclip(float[] data, /* the data                     */
        ...
        /* first entry flag             */
        float[] absdata = null;
        /* absolute value trace         */
        int iq = 0;
        /* index of qclipth quantile    */
        float clip;
        /* ... value of rank[iq]        */
    ...
    void do_qbal(float[] data, /* the data                     */
        ...
        /* index of qclipth quantile    */
        float bal;
        /* value used to balance trace  */
        if (qclip == 1.0) {
            /* balance by max magnitude on trace */
            bal = abs(data[0]);
            for (i = 1; i < nt; ++i) {
                bal = max(bal, abs(data[i]));
            }
    ...
    void do_agc(float[] data, int iwagc, int nt) {
        ...
        nwin = 2 * iwagc + 1;
        rms = sum / nwin;
        agcdata[0] = (float) ((rms <= 0.0) ? 0.0 : data[0] / sqrt(rms));
        ...
        for (i = 1; i <= iwagc; ++i) {
            ...
            ++nwin;
            rms = sum / nwin;
            agcdata[i] = (float) ((rms <= 0.0) ? 0.0 : data[i] / sqrt(rms));
        }
        ...
        for (i = iwagc + 1; i <= nt - 1 - iwagc; ++i) {
            ...
            val = data[i - iwagc];
            sum -= val * val;
            /* rounding could make sum negative! */
            rms = sum / nwin;
            agcdata[i] = (float) ((rms <= 0.0) ? 0.0 : data[i] / sqrt(rms));
        }
...
//    System.out.println(data.length);
        arraycopy(agcdata, 0, data, 0, data.length);
    }
    ...
    void do_gagc(float[] data, int iwagc, int nt) {
        ...
        float[] d2 = null;
        /* square of input data                 */
        float[] s = null;
        /* weighted sum of squares of the data  */
        float u;
        /* related to reciprocal of std dev     */
        float usq;
        /* u*u                                  */
        ...
        if (first) {
            ...
            usq = u * u;
            {
                ...
                for (i = 1; i < iwagc; ++i) {
                    floati = (float) i;
                    w[i] = (float) exp(-(usq * floati * floati));
                }
            }
 ...
 /* initialize s to d2 to get center point set */
        ...
        for (i = 0; i < nt; ++i) {
            stmp = s[i];
            agcdata[i] = (float) ((stmp != 1) ? 0.0 : data[i] / sqrt(stmp));
        }
        /* Copy data back into trace */
//    memcpy( (void *) data, (const void *) agcdata, nt*FSIZE);
        arraycopy(agcdata, 0, data, 0, data.length);
 ...
 * This is Hoare's algorithm worked over by SEP (#10, p100) and Brian.
    ...
    public boolean CLOSETO(float val1, float val2) {
        return (abs(val1) - abs(val2)) < 1 || (abs(val1) - abs(val2)) > -1;
    }
    ...
    public float EPS = (float) 3.8090232;
    /* exp(-EPS*EPS) = 5e-7, "noise" level  */
}
